...
// Replicate the exact hybridWorthinessDecision from ApplyPage
function hybridWorthinessDecision(
  ...
  const hasHighDepth = wellEvidenced.length > 0 ||
  ...
  if (isHardReject) {
    return {
      ...
      reasoning: 'Candidate does not meet minimum signal threshold.',
      ...
      recruiterNote: 'Does not meet minimum evidence threshold.',
  ...
  if (seniorityCheck === "mismatch") {
    ...
    algoReasons.push("Seniority mismatch");
  }
  ...
  if (isHighPotential && !algoWorthy) {
    return buildResult(false, 'High-potential candidate — core capabilities exceptional, domain gaps are trainable', 'Advanced credentials with production-scale impact.', Math.max(algoScore, 0.30), 'mixed_signals');
  }
  if (aiJudgment?.high_potential_override && !algoWorthy) {
    return buildResult(false, 'High-potential candidate — exceptional credentials warrant review', aiJudgment?.ai_reasoning || '', Math.max(algoScore, 0.30), 'mixed_signals');
  }
  if (!aiJudgment) {
    return buildResult(algoWorthy, algoWorthy ? 'Strong match — High Confidence' : 'Insufficient match — High Confidence', algoWorthy ? 'Algorithmic analysis indicates interview-worthy.' : 'Below algorithmic threshold.', algoScore, 'mixed_signals');
  }
  if (algoWorthy && aiWorthy) {
    const builderRatio = aiJudgment?.builder_verb_ratio || 0.5;
    if (builderRatio < 0.4) {
      return buildResult(false, 'Surface coverage without ownership depth', 'Skill keywords present but low ownership language detected.', algoScore, 'mixed_signals');
    }
    return buildResult(true, 'Strong match — High Confidence', aiJudgment?.ai_reasoning || 'Both assessments agree.', Math.max(algoScore, 0.75), mapConfidence(calibratedConf));
  }
  if (!algoWorthy && !aiWorthy) {
    if (isHighPotential) {
      return buildResult(false, 'High-potential candidate flagged despite threshold miss', 'Advanced credentials warrant manager review.', Math.max(algoScore, 0.25), 'mixed_signals');
    }
    return buildResult(false, 'Insufficient match — High Confidence', aiJudgment?.ai_reasoning || 'Both assessment layers agree.', Math.min(algoScore, 0.35), mapConfidence(calibratedConf));
  }
  if (!algoWorthy && aiWorthy) {
    return buildResult(false, 'Domain gap with strong transferable capability', aiJudgment?.ai_reasoning || '', algoScore, 'mixed_signals');
  }
  return buildResult(false, 'Surface coverage without ownership depth', aiJudgment?.ai_reasoning || '', algoScore, 'mixed_signals');
}
